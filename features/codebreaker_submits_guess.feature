Feature: code-breaker submits guess
  ...
  position of a number in the secret code, a - is added to the mark.
  
  Each position in the secret code can only be matched once. 
  For example, 1 guess of 1134 against a secret code of 1234 would
  get three plus signs: one for each of the exact matches in the first,
  third and fourth positions. The number match in the second position
  would be ignored.
